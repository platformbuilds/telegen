...
### Added
- **VMware scrape health gauges** — `vmware_scrape_collector_success` and `vmware_scrape_collector_duration_seconds` are now emitted for login, each collector unit, events, export (carry-forward), and all-collectors summary.
- **VMware esxcli collector coverage** — Added `vmware_esxcli_storage_driver` and `vmware_esxcli_host_nic_driver` collectors (both remain disabled by default).
...
### Changed
- **VMware perf sample emission shape** — `emitPerformanceMetrics` now emits one point per vCenter `PerfSampleInfo` sample with per-sample source timestamps, rather than collapsing a window into one averaged point.
- **VMware max sample derivation** — `Config.SampleCount` was removed; `PerfQuerySpec.MaxSample` is now derived from `collect_interval` and the queried vCenter `IntervalId` via `Config.MaxSamplesFor`.
...
### Fixed
- **VMware timestamp provenance skew signal** — `collector_clock_skew_seconds` now uses only source timestamps from vCenter samples and excludes inventory-cycle instants, so real clock drift is observable instead of being pinned near zero.
