...
// Per-CPU scratch space for large structs to avoid stack overflow
struct {
    __uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
    __uint(max_entries, 1);
    __type(key, u32);
    __type(value, llm_event_t);
} llm_event_scratch SEC(".maps");
...
struct {
    ...
    __type(key, u32);
    __type(value, struct llm_request);
} llm_request_scratch SEC(".maps");

// Helper to detect LLM provider from endpoint
// Note: This function will be used when full endpoint detection is implemented
static __always_inline __attribute__((unused)) u32 detect_llm_provider(const u8 *host, u16 port) {
...
static __always_inline void generate_request_id(u8 *id) {
    ...
    id[2] = '0' + ((ts / 100) % 10);
    id[3] = '0' + ((ts / 1000) % 10);
    // Include random value to ensure uniqueness
    id[4] = '-';
    id[5] = '0' + (random % 10);
    id[6] = '0' + ((random / 10) % 10);
    id[7] = '0' + ((random / 100) % 10);
    id[8] = '0' + ((random / 1000) % 10);
}
...
int BPF_UPROBE(http2_write_frame, void *frame_ctx) {
    (void)frame_ctx;  // Mark as used, actual context comes from BPF macro
    u32 zero = 0;

    struct llm_request *req = bpf_map_lookup_elem(&llm_request_scratch, &zero);
    llm_event_t *event = bpf_map_lookup_elem(&llm_event_scratch, &zero);
    if (!req || !event) {
        return 0;
    }
