...
func (c *Collector) Run(stop <-chan struct{}) {
    t := time.NewTicker(c.interval); defer t.Stop()
    for {
        select {
        case <-t.C:
            wr := &prompb.WriteRequest{}
            c.appendCPU(wr); c.appendMem(wr); c.appendNet(wr)
            if len(wr.Timeseries) > 0 { c.cb(wr) }
        case <-stop:
            return
        }
    }
...
func appendPoint(wr *prompb.WriteRequest, metric string, lbls []labels.Label, val float64) {
    ...
    for _, l := range lbls {
        if l.Name == "__name__" { labs = append(labs, prompb.Label{Name:"__name__", Value: metric}) } else { labs = append(labs, prompb.Label{Name: l.Name, Value: l.Value}) }
    }
...
func (c *Collector) appendCPU(wr *prompb.WriteRequest) {
    f, err := os.Open("/proc/stat"); if err != nil { return }; defer f.Close()
    sc := bufio.NewScanner(f)
    for sc.Scan() {
        line := sc.Text()
        if !strings.HasPrefix(line, "cpu") { continue }
        fs := strings.Fields(line); if len(fs) < 8 { continue }
        name := fs[0]
        var total float64
        for i := 1; i <= 8 && i < len(fs); i++ { total += atof(fs[i]) }
        user := atof(fs[1]); system := atof(fs[3])
        util := 0.0; if total > 0 { util = (user+system)/total }
...
func (c *Collector) appendMem(wr *prompb.WriteRequest) {
    f, err := os.Open("/proc/meminfo"); if err != nil { return }; defer f.Close()
    var memTotal, memAvail float64
    sc := bufio.NewScanner(f)
    for sc.Scan() {
        line := sc.Text()
        if strings.HasPrefix(line, "MemTotal:") { fmt.Sscanf(line, "MemTotal: %f kB", &memTotal) }
        if strings.HasPrefix(line, "MemAvailable:") { fmt.Sscanf(line, "MemAvailable: %f kB", &memAvail) }
    }
    if memTotal > 0 {
...
func (c *Collector) appendNet(wr *prompb.WriteRequest) {
    f, err := os.Open("/proc/net/dev"); if err != nil { return }; defer f.Close()
    sc := bufio.NewScanner(f); for i:=0;i<2 && sc.Scan(); i++ {}
    for sc.Scan() {
        line := strings.TrimSpace(sc.Text())
        iface, rest, ok := strings.Cut(line, ":"); if !ok { continue }
        fs := strings.Fields(rest); if len(fs) < 16 { continue }
        rx := atof(fs[0]); tx := atof(fs[8])
