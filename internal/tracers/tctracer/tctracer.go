...
package tctracer // import "github.com/mirastacklabs-ai/telegen/internal/tracers/tctracer"
...
//go:generate $BPF2GO -cc $BPF_CLANG -cflags $BPF_CFLAGS -target amd64,arm64 Bpf ../../../bpf/tctracer/tctracer.c -- -I../../../bpf
...
func (p *Tracer) Run(ctx context.Context, _ *ebpfcommon.EBPFEventContext, _ *msg.Queue[[]request.Span]) {
	...
	if err := p.bpfObjects.Close(); err != nil {
		p.log.Debug("failed closing tc tracer BPF objects", "error", err)
	}
}
