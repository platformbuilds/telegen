...
import (
	...
	"github.com/mirastacklabs-ai/telegen/internal/storage/netapp/eseries"
...
// sharedMetrics / sharedLogs may be nil; legacy storage.otlp is used as fallback for metrics.
func NewManager(cfg storagedef.Config, sharedMetrics sdkmetric.Exporter, sharedLogs *sdklog.LoggerProvider, log *slog.Logger) (*Manager, error) {
	if log == nil {
		log = slog.Default()
	}
...
func (m *Manager) Start(ctx context.Context) error {
	...
	for _, cfg := range m.config.DellPowerStore {
		...
		if err != nil {
			m.log.Error("failed to create Dell PowerStore collector", "name", cfg.Name, "error", err)
			continue
		}
		if err := collector.Start(ctx); err != nil {
			m.log.Error("failed to start Dell PowerStore collector", "name", cfg.Name, "error", err)
			continue
		}
	...
	for _, cfg := range m.config.HPEPrimera {
		...
		if err != nil {
			m.log.Error("failed to create HPE Primera collector", "name", cfg.Name, "error", err)
			continue
		}
		if err := collector.Start(ctx); err != nil {
			m.log.Error("failed to start HPE Primera collector", "name", cfg.Name, "error", err)
			continue
		}
		m.collectors[cfg.Name] = collector
	...
	for _, cfg := range m.config.PureFlashArray {
		...
		if err != nil {
			m.log.Error("failed to create Pure FlashArray collector", "name", cfg.Name, "error", err)
			continue
		}
		if err := collector.Start(ctx); err != nil {
			m.log.Error("failed to start Pure FlashArray collector", "name", cfg.Name, "error", err)
			continue
		}
		m.collectors[cfg.Name] = collector
	...
	for _, cfg := range m.config.NetAppONTAP {
		collector, err := netapp.NewONTAPCollector(cfg, m.log, netapp.Options{LoggerProvider: m.sharedLogs})
		if err != nil {
			m.log.Error("failed to create NetApp ONTAP collector", "name", cfg.Name, "error", err)
			continue
		}
		if err := collector.Start(ctx); err != nil {
			m.log.Error("failed to start NetApp ONTAP collector", "name", cfg.Name, "error", err)
			continue
		}
		m.collectors[cfg.Name] = collector
	...
	for _, cfg := range m.config.NetAppESeries {
		collector, err := eseries.NewCollector(cfg, m.log)
		if err != nil {
			m.log.Error("failed to create NetApp E-Series collector", "name", cfg.Name, "error", err)
			continue
		}
		if err := collector.Start(ctx); err != nil {
			m.log.Error("failed to start NetApp E-Series collector", "name", cfg.Name, "error", err)
			continue
		}
		m.collectors[cfg.Name] = collector
		m.log.Info("initialized NetApp E-Series collector", "name", cfg.Name)
	}
	...
	m.running = true
	m.log.Info("storage metrics manager started", "collectors", len(m.collectors))
	return nil
}
...
func (m *Manager) collectAll(ctx context.Context) {
	...
	for name, collector := range collectors {
		...
		if err != nil {
			m.log.Warn("failed to collect metrics", "collector", name, "error", err)
			m.updateHealth(name, &storagedef.CollectorHealth{
				Status: storagedef.HealthStatusUnhealthy, LastCheck: time.Now(), LastError: err,
				ResponseTime: time.Since(collectStart),
			})
			continue
		}
		m.updateHealth(name, &storagedef.CollectorHealth{
			Status: storagedef.HealthStatusHealthy, LastCheck: time.Now(), LastSuccess: time.Now(),
			ResponseTime: time.Since(collectStart),
		})
		...
		if len(metrics) > 0 {
			if m.sharedMetrics != nil {
				...
				if err := m.exporter.Export(ctx, metrics); err != nil {
					m.log.Warn("failed to export metrics", "collector", name, "error", err)
				}
			}
		}
		totalMetrics += len(metrics)
		m.log.Debug("collected metrics", "collector", name, "count", len(metrics), "duration", time.Since(collectStart))
	}

	m.lastRun = time.Now()
	m.log.Info("collection cycle completed",
		"collectors", len(collectors), "metrics", totalMetrics, "duration", time.Since(start))
}
...
func (m *Manager) UnregisterCollector(name string) error {
	...
	if err := collector.Stop(context.Background()); err != nil {
		m.log.Warn("error stopping collector during unregister", "name", name, "error", err)
	}
