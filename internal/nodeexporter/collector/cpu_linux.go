...
func init() {
	Register(cpuCollectorName, true, NewCPUCollector)
}

// cpuCollector exports CPU metrics from /proc/stat.
...
var jumpBackDebugMessage = fmt.Sprintf("CPU Idle counter jumped backwards more than %f seconds, possible hotplug event, resetting CPU stats", jumpBackSeconds)

// NewCPUCollector returns a new Collector exposing CPU statistics.
func NewCPUCollector(cfg CollectorConfig) (Collector, error) {
	pfs, err := procfs.NewFS(cfg.Paths.ProcPath)
	if err != nil {
		return nil, fmt.Errorf("failed to open procfs: %w", err)
	}

	sfs, err := sysfs.NewFS(cfg.Paths.SysPath)
	if err != nil {
	...
	if err != nil {
		...
		cfg.Logger.Debug("couldn't open isolated file", "error", err)
	}

	cpuConfig := DefaultCPUCollectorConfig()
	if cfg.CPUConfig != nil {
		cpuConfig = *cfg.CPUConfig
	}
	...
	if err := c.compileIncludeFlags(); err != nil {
		return nil, fmt.Errorf("failed to compile flags/bugs include regexps: %w", err)
	}

	return c, nil
}

func (c *cpuCollector) compileIncludeFlags() error {
	if (c.cpuConfig.FlagsInclude != "" || c.cpuConfig.BugsInclude != "") && !c.cpuConfig.EnableInfo {
		c.cpuConfig.EnableInfo = true
		c.logger.Info("cpu.info has been enabled because flags-include or bugs-include was set")
	}

	var err error
	if c.cpuConfig.FlagsInclude != "" {
		c.cpuFlagsIncludeRegexp, err = regexp.Compile(c.cpuConfig.FlagsInclude)
		if err != nil {
			return err
		}
	}
	if c.cpuConfig.BugsInclude != "" {
		c.cpuBugsIncludeRegexp, err = regexp.Compile(c.cpuConfig.BugsInclude)
		if err != nil {
			return err
		}
	}
...
func (c *cpuCollector) updateInfo(ch chan<- prometheus.Metric) error {
	...
	if len(info) != 0 {
		cpu := info[0]
		if err := c.updateFieldInfo(cpu.Flags, c.cpuFlagsIncludeRegexp, c.cpuFlagsInfo, ch); err != nil {
			return err
		}
		if err := c.updateFieldInfo(cpu.Bugs, c.cpuBugsIncludeRegexp, c.cpuBugsInfo, ch); err != nil {
			return err
		}
	}
...
func (c *cpuCollector) updateFieldInfo(valueList []string, filter *regexp.Regexp, desc *prometheus.Desc, ch chan<- prometheus.Metric) error {
	if filter == nil {
		return nil
	}
...
// updateThermalThrottle reads /sys/devices/system/cpu/cpu* and exposes thermal throttle statistics.
func (c *cpuCollector) updateThermalThrottle(ch chan<- prometheus.Metric) error {
	cpus, err := filepath.Glob(c.pathConfig.SysFilePath("devices/system/cpu/cpu[0-9]*"))
	if err != nil {
		return err
	}
	...
	for _, cpu := range cpus {
		...
		physicalPackageID, err = readUintFromFile(filepath.Join(cpu, "topology", "physical_package_id"))
		if err != nil {
			c.logger.Debug("CPU is missing physical_package_id", "cpu", cpu)
			continue
		}
		// topology/core_id
		coreID, err = readUintFromFile(filepath.Join(cpu, "topology", "core_id"))
		if err != nil {
...
// updateIsolated exports isolation level metrics.
...
// updateOnline exports online status metrics.
...
// readUintFromFile reads a single uint64 from a file.
