...
import (
    ...
    "go.opentelemetry.io/contrib/exporters/otlp/otlplog/otlploggrpc"
    "go.opentelemetry.io/contrib/exporters/otlp/otlplog/otlploghttp"
...
func New(ctx context.Context, o TraceOpts) (*Clients, error) {
    ...
    if o.GRPC.Enabled {
        ...
        if err == nil { tp = sdktrace.NewTracerProvider(sdktrace.WithBatcher(exp)) }
    }
    ...
    if tp == nil { return nil, errors.New("no OTLP trace endpoint usable") }
    ...
    if o.GRPC.Enabled {
        ...
        if o.TLS.Enable && !o.GRPC.Insecure {
            creds, err := buildTLS(o.TLS.CAFile, o.TLS.CertFile, o.TLS.KeyFile, o.TLS.InsecureSkipVerify)
            if err == nil { lopts = append(lopts, otlploggrpc.WithTLSCredentials(creds)) } else { lopts = append(lopts, otlploggrpc.WithInsecure()) }
        } else { lopts = append(lopts, otlploggrpc.WithInsecure()) }
        lexp, err := otlploggrpc.New(ctx, lopts...)
        if err == nil { lp = sdklog.NewLoggerProvider(sdklog.WithProcessor(sdklog.NewBatchProcessor(lexp))) }
    }
    ...
    if lp == nil { return nil, errors.New("no OTLP log endpoint usable") }

    return &Clients{ Trace: tp, Log: lp, close: func(ctx context.Context) error {
        var e1, e2 error
        if lp != nil { e1 = lp.Shutdown(ctx) }
        if tp != nil { e2 = tp.Shutdown(ctx) }
        if e1 != nil { return e1 }
        return e2
    }}, nil
}
...
func buildTLS(ca, cert, key string, insecure bool) (credentials.TransportCredentials, error) {
    ...
    if ca != "" {
        b, err := ioutil.ReadFile(ca); if err != nil { return nil, err }
        pool := x509.NewCertPool()
        if !pool.AppendCertsFromPEM(b) { return nil, errors.New("bad ca") }
        cfg.RootCAs = pool
    }
    if cert != "" && key != "" {
        crt, err := tls.LoadX509KeyPair(cert, key); if err != nil { return nil, err }
