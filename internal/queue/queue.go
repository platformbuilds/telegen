...
func (q *Ring[T]) PopBatch(maxN int, maxWait time.Duration) []Item[T] {
    ...
    for {
        q.mu.Lock()
        if q.sz > 0 {
            n := maxN; if n > q.sz { n = q.sz }
            out := make([]Item[T], n)
            for i := 0; i < n; i++ { out[i] = q.buf[(q.head+i)%q.cap] }
            q.head = (q.head + n) % q.cap; q.sz -= n; q.mu.Unlock()
            return out
        }
        q.mu.Unlock()
...
func (q *Ring[T]) DropExpired(maxAge time.Duration) (dropped int) {
    ...
    for q.sz > 0 {
        it := q.buf[q.head]
        if now.Sub(it.Enqueue) <= maxAge { break }
        q.head = (q.head + 1) % q.cap; q.sz--; dropped++
    }
    if dropped > 0 { q.dropped += uint64(dropped); q.coalesceLog(ReasonMaxAge) }
    return
}
...
func (q *Ring[T]) coalesceLog(reason DropReason) {
    if time.Since(q.lastDrop) > 10*time.Second {
        ...
        q.lastDrop = time.Now()
        if q.onDrop != nil { q.onDrop(q.dropped, reason) }
    }
}
