...
const testTraceFlagSampled = uint8(1)
...
func TestReadBPFTraceAsSpanGoChannelLinkEvent(t *testing.T) {
	...
	assert.Equal(t, testTraceFlagSampled, receiverSpan.Links[0].TraceFlags)
...
func TestPendingSpanLinksDeduplicates(t *testing.T) {
	...
	link := request.SpanLink{TraceID: testTraceID(3), SpanID: testSpanID(4), TraceFlags: testTraceFlagSampled}
