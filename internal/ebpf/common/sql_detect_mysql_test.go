...
func TestMySQLParsing(t *testing.T) {
	for _, ts := range []struct {
		...
		{
			name:  "Valid Query",
			bytes: []byte{0x09, 0x00, 0x00, 0x01, 0x03, 0x73, 0x65, 0x6c, 0x65, 0x63, 0x74, 0x20, 0x31},
			valid: true,
			result: mySQLHdr{
				length:  0x09,
				command: 0x3,
...
func TestIsMySQLRejectsPostgresQuery(t *testing.T) {
	postgresQuery := buildPostgresFrame(kPostgresQuery, append([]byte("SELECT 1"), 0))
	assert.False(t, isMySQL(postgresQuery), "postgres query frame must not be classified as mysql")
}
