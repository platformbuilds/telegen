...
func (l *Loader) Run(ctx context.Context) error {
	...
	if err != nil {
		log.Printf("ebpf: parse failed: %v; simulator fallback", err)
		return l.simulate(ctx)
	}
	coll, err := ebpf.NewCollection(spec)
	if err != nil {
		log.Printf("ebpf: load failed: %v; simulator fallback", err)
		return l.simulate(ctx)
	}
	defer coll.Close()
	...
	if err != nil {
		log.Printf("ebpf: kprobe send: %v; simulator fallback", err)
		return l.simulate(ctx)
	}
	recv, err := link.Kprobe("tcp_cleanup_rbuf", coll.Programs["k_tcp_cleanup_rbuf"], nil)
	if err != nil {
		_ = send.Close()
		log.Printf("ebpf: kprobe recv: %v; simulator fallback", err)
		return l.simulate(ctx)
	}
	...
	if !ok {
		return errors.New("events ringbuf not found")
	}
	rdr, err := ringbuf.NewReader(rb)
	if err != nil {
		log.Printf("ebpf: ringbuf reader: %v; simulator fallback", err)
		return l.simulate(ctx)
	}
	l.reader = rdr
	defer rdr.Close()
	...
	for {
		...
		// Record is automatically handled by the ringbuf reader
	}
}
